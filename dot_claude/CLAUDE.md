...
- `r` = 確認（Review）
- `u` = 元に戻す（Undo）
- `ut` = 深く考える（ultrathink - 複雑な問題に対して詳細な分析と検討を行う）
...
- ❌ TODO リストに未完了タスクがある場合は絶対に使用しない
- ❌ 作業の進捗報告中や、まだ続きがある場合は使用しない
- ❌ ユーザーが提示した TODO や作業リストに未完了項目がある場合は使用しない

**重要な確認事項**：

合い言葉を使用する前に、必ず以下を確認すること：
1. Update Todos に ☐ マークの項目が残っていないか
2. 「次は〜に進みます」のような継続予定の記述をしていないか
3. Phase や Step など段階的な作業で未完了の段階が残っていないか
